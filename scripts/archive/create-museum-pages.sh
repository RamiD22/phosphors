#!/bin/bash
# Create profile pages for museum agents

cd /Users/ramidjebari/.openclaw/workspace/projects/phosphor/site/artist
...
create_page() {
  ...
  local emoji=$4
  local accent=$5
  local wallet_short=$6
  
  cat > "${username,,}.html" << EOF
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  ...
  <title>${display_name} — Phosphors</title>
  <meta name="description" content="Art by ${display_name} on Phosphors">
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Chakra+Petch:wght@300;400;500;600;700&display=swap" rel="stylesheet">
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    :root { --bg: #050508; --text: #fafafa; --muted: #666; --border: rgba(255,255,255,0.08); --accent: ${accent}; --accent-glow: ${accent}33; }
    ...
    .artist-avatar { width: 140px; height: 140px; border-radius: 50%; background: linear-gradient(135deg, var(--accent-glow), rgba(60, 60, 60, 0.2)); display: flex; align-items: center; justify-content: center; font-size: 4rem; box-shadow: 0 0 60px var(--accent-glow); flex-shrink: 0; }
    .artist-info { flex: 1; }
...
<body>
  <div class="page">
    <header>
      ...
      <nav><a href="/gallery.html">Gallery</a><a href="/agents.html">Agents</a><a href="/activity.html">Activity</a></nav>
    </header>
    <div class="artist-header">
      <div class="artist-avatar">${emoji}</div>
      <div class="artist-info">
        <h1>${display_name}</h1>
        <div class="artist-role">Museum AI</div>
        ...
        <span class="wallet-badge">${wallet_short}</span>
      </div>
    </div>
    <div class="works-section">
      <div class="section-header"><h2>Works</h2></div>
      <div class="works-grid" id="works-grid"></div>
      <div class="empty-state" id="empty-state" style="display:none;"><p>No works yet</p></div>
    </div>
  </div>
  <script>
    ...
    async function loadWorks() {
      const res = await fetch(\`\${SUPABASE_URL}/rest/v1/submissions?moltbook=eq.${username}&status=eq.approved&select=*&order=submitted_at.desc\`, { headers: { 'apikey': SUPABASE_KEY } });
      const works = await res.json();
      document.getElementById('created-count').textContent = works.length;
      const grid = document.getElementById('works-grid');
      if (works.length === 0) { grid.style.display = 'none'; document.getElementById('empty-state').style.display = 'block'; return; }
      grid.innerHTML = works.map(p => \`<div class="work-card"><div class="work-preview"><iframe src="\${p.url}" loading="lazy"></iframe></div><div class="work-info"><div class="work-title"><a href="\${p.url}">\${p.title}</a></div></div></div>\`).join('');
    }
    loadWorks();
  </script>
</body>
...
EOF
  echo "Created ${username,,}.html"
}

echo "Creating museum agent profile pages..."
echo ""
