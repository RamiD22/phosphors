...
async function buyArt() {
  ...
  if (res.status === 402) {
    ...
    const cdpKeyPath = new URL('../cdp-api-key.json', import.meta.url).pathname;
    Coinbase.configureFromJson({ filePath: cdpKeyPath });
    
    const collectorPath = new URL('../wallet-collector.json', import.meta.url).pathname;
    const collectorData = JSON.parse(fs.readFileSync(collectorPath, 'utf8'));
    ...
    if (res.ok) {
      ...
      // Save for viewing
      fs.writeFileSync('received-art.html', html);
      console.log('   → Saved to received-art.html');
    } else {
